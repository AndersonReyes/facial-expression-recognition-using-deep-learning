...
def conv2d(x, filter_shape):
    ...
    B = tf.Variable(tf.constant(0.1), [filter_shape[-1]])
...
conv_layer2 = relu(conv2d(conv_layer1, [5, 5, 32, 64]))
...
fc_l3 = fully_connected(max_pool_layer2, [12 * 12 * 128, 1000])
outlayer = output_layer(fc_l3, [1000, 7])

entropy_cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=outlayer, labels=Y))
optimiser = tf.train.AdamOptimizer(learning_rate=alpha).minimize(entropy_cost)
correct_prediction = tf.equal(tf.argmax(Y, 1), tf.argmax(outlayer, 1))
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for i in range(n_batches):
            idx = i*n_batches
            batch_x = X_train[idx: idx + n_batches]
            batch_y = Y_train[idx: idx + n_batches]
