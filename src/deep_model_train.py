...
X_train, X_test, Y_train, Y_test = train_test_split(
    X_all, Y_all, test_size=.20, random_state=20)
...
#  ARCHITECTURE
layer1 = tf.layers.conv2d(input_shaped, filters=64, kernel_size=[
                          5, 5], padding='same', activation=tf.nn.relu, name='l1_conv2d')
layer1 = tf.layers.max_pooling2d(
    layer1, pool_size=[2, 2], strides=2, name='l1_maxpool')
layer1 = tf.layers.dropout(layer1, rate=0.5, name='l1_dropout')

layer2 = tf.layers.conv2d(layer1, filters=64, kernel_size=[
                          5, 5], padding='same', activation=tf.nn.relu, name='l2_conv2d')
layer2 = tf.layers.max_pooling2d(
    layer2, pool_size=[2, 2], strides=2, name='l2_maxpool')
layer2 = tf.layers.dropout(layer2, rate=0.5, name='l2_dropout')

layer3 = tf.layers.conv2d(layer2, filters=64, kernel_size=[
                          5, 5], padding='same', activation=tf.nn.relu, name='l3_conv2d')
layer3 = tf.layers.max_pooling2d(
    layer3, pool_size=[2, 2], strides=2, name='l3_maxpool')
layer3 = tf.layers.dropout(layer1, rate=0.5, name='l3_dropout')

layer4 = tf.layers.conv2d(layer3, filters=128, kernel_size=[
                          5, 5], padding='same', activation=tf.nn.relu, name='l4_conv2d')
layer4 = tf.layers.max_pooling2d(
    layer4, pool_size=[2, 2], strides=2, name='l4_maxpool')
layer4 = tf.layers.dropout(layer4, rate=0.5, name='l4_dropout')

layer5 = tf.layers.conv2d(layer4, filters=128, kernel_size=[
                          5, 5], padding='same', activation=tf.nn.relu, name='l5_conv2d')
layer5 = tf.layers.max_pooling2d(
    layer5, pool_size=[2, 2], strides=2, name='l5_maxpool')
layer5 = tf.layers.dropout(layer5, rate=0.5, name='l5_dropout')


flattened = tf.reshape(layer5, [-1, 6 * 6 * 128], name='flattened')
dense1024 = tf.layers.dense(flattened, units=1024,
                            activation=tf.nn.relu, name='l6_dense_1024')
dropout = tf.layers.dropout(dense1024, rate=0.5, name='l6_dropout')
dense512 = tf.layers.dense(
    dropout, units=512, activation=tf.nn.relu, name='l7_dense_512')
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for i in range(n_batches):
            ...
            _, result = sess.run([optimiser, entropy_cost], feed_dict={
                                 input: batch_x, y: batch_y})
            avg_cost += result / n_batches

        test_accuracy = sess.run(accurary, feed_dict={
                                 input: X_test, y: Y_test})
        if epoch % logging_steps == 0:
            print('\nEpoch:' + str(epoch + 1) + ' cost = {:.3f}'.format(
                avg_cost) + ' test accuracy: {:.3f}'.format(test_accuracy))

        file.write(
            str(epoch + 1) + ',{:.3f}'.format(avg_cost) + ',{:.3f}\n'.format(test_accuracy))

    print('\nTraining Complete')
    print('\naccurary: {0}\n'.format(
        sess.run(accurary, feed_dict={input: X_test, y: Y_test})))
    file.close()
